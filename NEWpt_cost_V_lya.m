...
cost_delay_V_lya=zeros(pn,3); % def V,cost,delay
for pt=1:pn
    load(strcat('./data/main_lya_data_Vmax_E',num2str(pt-1),'00'))
    b1=data_lya(1:dc,:);              %ai
    ...
    cost_delay_V_lya(pt,1)=V;
    cost_delay_V_lya(pt,2)=avg_cost(end);
    cost_delay_V_lya(pt,3)=avg_delay(end);
end
...
for pt=0:2:8
    load(strcat('./data/main_lya_data_Vmax_E',num2str(pt),'0patch'))
    b1=data_lya(1:dc,:);              %ai
    b2=data_lya(dc+1:2*dc,:);     %bi
    b3=data_lya(2*dc+1:3*dc,:);  %Di
    b4=data_lya(3*dc+1:4*dc,:);  %ri
...
cost_delay_V_lya = [cost_delay_V_lya(1,:);cost_delay_V_lya_p(2:end,:);cost_delay_V_lya(2:end,:)];
%% --------------------------gre--------------------------
...
figure(1)
subplot(121)
plot(cost_delay_V_lya(:,1),cost_delay_V_lya(:,2))
hold on
% plot(cost_delay_V_lya(:,1),cost_delay_V_gre)    % including greedy algorithm
% hold on
% axis([0 inf 0 inf])
subplot(122)
plot(cost_delay_V_lya(:,1),cost_delay_V_lya(:,3))
hold on

% save ./data/pt_cost_V_lya_patch cost_delay_V_lya cost_delay_V_gre