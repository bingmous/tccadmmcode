...
data_ad_TCC = zeros(4*dc, 30); % for one time slot
% data_flag_r = zeros(30, 1);
% data_flag_s = zeros(30, 1);
% data_eps_pri = zeros(30, 1);
% data_eps_dual = zeros(30, 1);
...
% %             f_ad1(1)=factor(dc*1-dc+d)+dk(dc*1-dc+d)-rho*zk(dc*1-dc+d);
            f_ad1(1)=factor(dc*1-dc+d)-gk(d);  % gk=yk*I-yk_bar*I-1/rho*dk+A(t)/N
...
%             end
        %% -------------------function 3-----------------------
        yk_bar=sum(yk_temp,2)/dc;
% %         dk=dk+rho*(yk(1:dc)-zk); % yk update
        dk=dk+rho*(yk_bar'*I-A(t)/dc);
        zk0=zk; % record old zk: yk*I-yk_bar*I+A(t)/dc
        for d=1:dc
            gk(d)=yk_temp(:,d)'*I-yk_bar'*I-dk/rho+A(t)/dc;
            zk(d)=yk_temp(:,d)'*I-yk_bar'*I+A(t)/dc;
        end
        %% -----stop condition--------
% %         flag_r=norm(yk(1:dc)-zk);  % primal residual
        flag_r=dc*norm(yk_bar'*I-A(t)/dc);
        flag_s=rho*norm((zk-zk0)); % dual residual
        eps_pri=eps_abs+eps_rel*max(norm(yk(1:dc)),norm(zk));
%             max(norm(yk),norm(zk)) % 300
...
%             norm(dk) % 600
        ...
        data_eps_pri(iter) = eps_pri;
        data_eps_dual(iter) = eps_dual;
        if iter==1000
            break
        end
...
ts_main_ad=toc(st)
iter_all
    disp(['time:',num2str(ts_main_ad),' total iter:',num2str(iter_all), 'avg iter:', num2str(iter_all/tmax)]);
save ./data/main_ad2_sim2_data_TCC_convergence1
% 
% % equality constrain
% figure(19)
% b1=data_ad(1:dc,:); 
